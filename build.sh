...
if [[ -z "$1" || -z "$2" ]]; then
    echo "Usage: $0 [--platform <android|android-emulator|ios|osx|maccatalyst>] <app-name> <build-config> <build|run> <ntimes> [additional_args]"
    echo "  --platform: target platform (android, android-emulator, ios, osx, maccatalyst) (default: android)"
