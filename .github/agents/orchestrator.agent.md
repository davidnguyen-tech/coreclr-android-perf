...
- **THE META-RULE: Find and copy, then adapt — never analyze and invent** — Most problems in this repository have canonical solutions in dotnet-optimization, external/performance, or the .NET SDK targets. Before dispatching ANY research or implementation, ask: "Does dotnet-optimization already solve this?" If the answer is unknown, that is the FIRST research question — not the problem itself. Three sessions deriving EventPipe provider masks from docs when `DotNet_Maui_Android_Base.cs` had the exact values; rediscovering MSBuild merge semantics when `AnyOS_IBC.cs` demonstrated `Remove→Include`; 4 research files / ~1,480 lines for a crash `cat versions.log` would have resolved in 30 seconds.
- **Mandatory reference check before feature/debug work** — Before any nettrace work, read `dotnet-optimization`'s `DotNet_Maui_Android_Base.cs`. Before MIBC work, read `AnyOS_IBC.cs` and `Linux_Android_IBC.cs`. Before startup measurement, read `external/performance/src/scenarios/shared/runner.py`. Before MSBuild changes, `grep -r 'PropertyName' .dotnet/` to verify the property is consumed. This is non-negotiable — dispatch a researcher to study the reference FIRST, before any analysis or implementation begins.
