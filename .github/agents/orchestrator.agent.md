...
- **xharness mlaunch is incompatible with iOS 17+ physical devices** — xharness bundles Xamarin's legacy `mlaunch` tool which uses deprecated `MobileDevice.framework` APIs. It cannot detect or communicate with devices running iOS 17+. Use `xcrun devicectl` (Xcode 15+) for all physical iOS device operations. This means runner.py's iOS device path does not work — dedicated measurement scripts using `xcrun devicectl` are required, following the same pattern as macOS, Mac Catalyst, and iOS Simulator.
