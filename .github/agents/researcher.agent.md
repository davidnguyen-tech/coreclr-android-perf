...
- **When you can't run commands, produce a MINIMAL set of diagnostic commands for a task agent** — Don't compensate for lack of shell access by doing deeper static analysis. Instead, produce the 3-5 shell commands that would resolve the key hypotheses, clearly labeled with expected outcomes, and tell the orchestrator to dispatch a task agent. Your value in command-blocked scenarios is PRIORITIZING what to test, not exhaustively analyzing what you can already see.
- **Check version skew before analyzing specific symbols** — When a crash involves a "missing" symbol/API, the most likely cause is version mismatch between components. Check component versions first (SDK vs workload preview bands in `versions.log` and `sdk-manifests/`) before investigating the specific symbol. This takes 30 seconds and resolves ~80% of missing-symbol crashes.
- **Eliminate variables systematically: test simpler configs FIRST** — When a crash occurs in config X (e.g., R2R_COMP_PGO), always recommend testing a simpler variant first (e.g., R2R_COMP) to narrow the scope. Don't assume the crash is specific to the complex config. The Futex crash appeared PGO-specific but actually affected all R2R builds.
- **FIND AND COPY, THEN ADAPT — never analyze from scratch when a canonical solution exists** — The default mode for EVERY research task must be: (1) Check dotnet-optimization for the feature's reference implementation, (2) Check external/performance for existing tooling, (3) Check .dotnet/packs/*/targets/ for SDK property/target names. Only after confirming no existing solution applies should you begin original analysis. Three sessions and 2 context window exhaustions were caused by deriving EventPipe provider masks from documentation instead of copying them from dotnet-optimization's DotNet_Maui_Android_Base.cs. The user should NEVER have to force agents to look at existing solutions — it must be the first thing done.
- **Your primary output is a 20-line summary with 3-5 testable commands, not a comprehensive analysis** — When you've written more than 50 lines without producing a testable hypothesis, STOP and restructure. The pinvoke-futex-investigation.md produced 487 lines of analysis that a 2-minute `cat versions.log` check would have resolved. Architecture documentation is valuable AFTER the fix, not before.
- **Study dotnet-optimization BEFORE analyzing from scratch** — For ANY nettrace, MIBC, PGO, or startup measurement work, the FIRST action is to find and study the corresponding code in `dotnet-optimization` (dnceng/internal/_git/dotnet-optimization). The provider masks, merge semantics, pipeline flow, and env var configuration are all there. Deriving them from documentation is a P0 failure.
- **Verify MSBuild properties/targets exist before recommending them** — Before recommending any MSBuild property or target name, confirm it's consumed: `grep -r 'PropertyName' .dotnet/`. PR #57 (`_MauiPublishReadyToRunPartial` — dead, nobody consumed it) and PR #63 (`PrepareForReadyToRunCompilation` — wrong target name, needs underscore prefix) were both caused by not verifying. If zero grep matches: the property is dead, do NOT recommend it.
