...
- **"Tool exists" ≠ "tool works on our target"** — runner.py had iOS support in code, but it depended on xharness mlaunch which is broken on iOS 17+. Always empirically verify the full tool chain on the actual target device/OS before reporting compatibility.
