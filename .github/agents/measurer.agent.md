...
- **iOS Watchdog events need 5s to flush** — After launching an app on a physical iOS device, wait at least 5s before running `sudo log collect --device`. The unified log system needs time to flush all 4 SpringBoard Watchdog events. 3s causes ~50% parse failures. This matches dotnet/performance's runner.py timing.
