...
- **Always capture PID from `xcrun devicectl device process launch`** — The launch command outputs the PID of the started process. Capture it and use `xcrun devicectl device process terminate --device <UDID> --pid <PID>` for reliable termination. Terminating by bundle ID does not work with `devicectl`.
...
- **macOS `log` command has no `help` subcommand** — `log help` returns exit code 64 (unrecognized subcommand). Use `log --help` or `log <subcommand> --help` instead. When writing sudo checks, always test with the exact subcommand that will be used (e.g., `sudo -n /usr/bin/log collect --help`), and verify the check command actually returns exit 0 before shipping.
- **Don't suppress stderr on sudo commands with `2>/dev/null`** — When `sudo` fails (wrong password, no NOPASSWD entry), the error message is the only diagnostic. Always capture or display stderr from sudo commands, then provide a helpful error message pointing the user to the fix.
- **`xcrun devicectl list devices` includes the host Mac** — The host Mac appears as a CoreDevice with `transportType: 'localNetwork'`. When filtering for physical iOS devices, use `transportType == 'wired'` AND `hardwareProperties.platform in ('iOS', 'iPadOS')`. Never include `localNetwork` or `wifi` in the transport filter.
- **`xcrun devicectl --json-output /dev/stdout` produces unparseable output** — The pipe mixes human-readable text with JSON. Always use a temp file: `xcrun devicectl list devices --json-output "$tmpfile"`, then parse `"$tmpfile"`. Never pipe devicectl JSON through stdin.
- **`xcrun xctrace list devices` output format is misleading** — Individual simulator entries (e.g., `iPhone 15 Pro (UUID)`) do NOT contain the word "Simulator". `grep -v "Simulator"` only removes the `== Simulators ==` section header. Never use xctrace as a fallback for device discovery — use `xcrun devicectl` exclusively.
- **`xcrun devicectl list devices` writes a text table to stdout even with `--json-output`** — The `--json-output <file>` flag writes JSON to the specified file but ALSO writes a human-readable table to stdout. When calling this inside a `$()` capture (e.g., in a function that echoes a UDID), always redirect stdout: `xcrun devicectl list devices --json-output "$file" >/dev/null 2>&1`. Otherwise the text table contaminates the captured output.
...
- **runner.py handles iOS device measurement end-to-end** — For physical iOS devices, the dotnet/performance runner.py (`external/performance/src/scenarios/shared/runner.py`) handles install (xharness), launch (xharness mlaunch), log collection (`sudo log collect --device`), Watchdog parsing, and timing computation. Don't reinvent this — just ensure the scenario directory exists and the routing reaches test.py.
- **xharness mlaunch cannot communicate with iOS 17+ devices** — The `xharness apple mlaunch` command uses Xamarin's legacy device communication protocol. For physical iOS devices running iOS 17+, use `xcrun devicectl` exclusively. The dotnet/performance runner.py iOS device path is NOT functional on modern iOS — always route to the dedicated `ios/measure_device_startup.sh` script.
