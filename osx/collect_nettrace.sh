...
# macOS apps run locally — no dsrouter device bridge is needed.
...
print_usage() {
    ...
    echo ""
    echo "Options:"
    echo "  --platform PLATFORM      Target platform (default: osx)"
    ...
    echo ""
    echo "Output: traces/<app>_<config>/<app>-osx-<config>-<YYYYMMDD-HHMMSS>.nettrace"
    exit 1
}
...
TRACE_FILE="$TRACE_DIR/${SAMPLE_APP}-osx-${BUILD_CONFIG}-${TRACE_TIMESTAMP}.nettrace"
