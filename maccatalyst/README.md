# Mac Catalyst Performance Measurement Support

Mac Catalyst-specific build configurations, workarounds, and tooling for CoreCLR startup performance measurement on Apple Silicon Macs running iPad apps via Mac Catalyst.
...
Mac Catalyst uses 6 build configurations. Non-composite ReadyToRun (`R2R`) is **not available** on Apple platforms because the MachO binary format only supports Composite R2R images.
...
## Mac Catalyst-Specific MSBuild Properties
...
| `PublishReadyToRunComposite` | Enables composite R2R. Must be `True` for Mac Catalyst (non-composite not supported). |

## App Templates

Mac Catalyst is **MAUI-only** — there is no standalone `dotnet new` template for Mac Catalyst (unlike `dotnet new ios` or `dotnet new macos`). All Mac Catalyst apps are generated from the MAUI template with `TargetFrameworks` rewritten to include `net11.0-maccatalyst`.

Available sample apps:

- **`dotnet-new-maui`** — .NET MAUI cross-platform app with `TargetFrameworks` set to `net11.0-maccatalyst`.
- **`dotnet-new-maui-samplecontent`** — .NET MAUI app with sample content pages, also targeting `net11.0-maccatalyst`.
...
```bash
# Install SDK, workloads, and tools for Mac Catalyst
./prepare.sh --platform maccatalyst

# Generate sample Mac Catalyst apps (MAUI-based only)
./generate-apps.sh --platform maccatalyst
```

### Building

```bash
# Build MAUI app with CoreCLR JIT
./build.sh --platform maccatalyst dotnet-new-maui CORECLR_JIT build 1

# Build MAUI app with R2R Composite
./build.sh --platform maccatalyst dotnet-new-maui R2R_COMP build 1
```
...
./measure_startup.sh dotnet-new-maui CORECLR_JIT --platform maccatalyst

# Measure all Mac Catalyst apps across all configs
./measure_all.sh --platform maccatalyst

# Measure with fewer iterations for quick testing
./measure_all.sh --platform maccatalyst --startup-iterations 3
```

### App Size Reporting

```bash
# Print sizes of built Mac Catalyst .app bundles
./maccatalyst/print_app_sizes.sh

# Detailed view showing largest files in each bundle
./maccatalyst/print_app_sizes.sh -detailed
```

## Package Discovery

Mac Catalyst apps are built as `.app` directory bundles (not single files like Android APKs). The measurement scripts use:
...
- Bundles are found under `apps/<app-name>/bin/Release/net11.0-maccatalyst/maccatalyst-arm64/`
